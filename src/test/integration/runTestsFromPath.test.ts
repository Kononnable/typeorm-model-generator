...
import { createDriver, dataCollectionPhase } from "../../lib";
import * as GTU from "../utils/GeneralTestUtils";
import { Entity } from "../../lib/models";
import IConnectionOptions from "../../lib/IConnectionOptions";
import modelCustomizationPhase from "../../lib/ModelCustomization";
import modelGenerationPhase from "../../lib/ModelGeneration";
...
async function runTestsFromPath(
    ...
    const files = fs.readdirSync(
        path.resolve(process.cwd(), "src", testPartialPath)
    );
    if (isDbSpecific) {
...
export function compileGeneratedModel(
    ...
    if (lintGeneratedFiles) {
        const cli = new CLIEngine({
            configFile: "src/test/configs/.eslintrc.js"
        });
...
async function prepareTestRuns(
    ...
    const driver = await createDriver(dbDriver);
